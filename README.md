...
1. 한글은 글가드 두벌식, 영어는 ABC를 사용하고 macOS가 입력 소스 전환을 담당합니다.
...
2. `+`를 눌러 한국어 아래의 `글가드 두벌식`과 영어 아래의 `ABC` 추가
3. 메뉴 막대 입력 메뉴에서 `글가드 두벌식` 선택

메뉴 막대 입력 메뉴 또는 macOS의 입력 소스 전환 단축키로 `ABC ↔ 글가드 두벌식`을
...
업데이트가 설정된 배포본에서는 메뉴 막대의 글가드 아이콘 → `업데이트 확인…`으로
새 버전을 확인합니다. 앱이 시작될 때 백그라운드에서 한 번 자동 확인하며 새 버전이
있을 때만 안내합니다. 설정의 `앱 시작 시 업데이트 확인`에서 끌 수 있습니다.
설치는 사용자 확인과 관리자 인증을 거칩니다.

설치 전에 입력 중인 글자를 확정하고 ABC로 전환하세요. 조합 중인 글자가 남거나
현재 소스가 ABC가 아니면 업데이트를 위한 종료를 막습니다. 설치 후 글가드를 다시
선택하고, 새 버전이 반영되지 않으면 작업 저장 후 로그아웃·로그인하세요.
업데이터가 없는 기존 버전은 업데이트 기능을 포함한 PKG를 한 번 수동 설치해야 합니다.
피드·공개키를 지정하지 않은 개발 빌드에서는 업데이트 메뉴와 설정이 비활성화됩니다.
배포 준비와 검증 절차는 [업데이트 배포 문서](docs/updates.md)를 참고하세요.

## 권장 사용법: ABC + 글가드 두벌식 + Karabiner

영어는 **ABC**, 한글은 **글가드 두벌식**, Caps Lock 전환은 **Karabiner-Elements**가
담당하는 구성을 권장합니다. Karabiner는 선택 사항이며, 없어도 시스템 단축키로
글가드를 사용할 수 있습니다.

### 1. macOS 입력 소스 설정

- 시스템 설정 → 키보드 → 텍스트 입력 → 편집에서 `ABC`와 `글가드 두벌식`을 추가합니다.
  두 소스만 사용하면 전환 대상을 예측하기 쉽습니다. 다른 입력 소스가 필요하면
  유지해도 되지만, ‘이전 입력 소스’가 항상 ABC/글가드 두벌식인 것은 아닙니다.
...
> 설치해 줘. 영어 ABC와 한글 글가드 두벌식을 함께 쓰고, Caps Lock은 카라비너로 입력
...
한자 변환, 글가드 자체 Caps Lock 전환, 앱별 모드 기억은 현재 포함하지 않습니다.
Caps Lock 전환은 위 Karabiner 설정으로 구성할 수 있습니다.
다른 자판 배열과 조합 옵션의 진행 항목은 [`ROADMAP.md`](ROADMAP.md)에서 관리합니다.

메뉴 막대의 글가드 아이콘에서 `설정…`을 열면 연속 자음의 쌍자음 조합을
켜거나 끌 수 있습니다. 켜면 `ㄱㄱ → ㄲ`, 끄면 `ㄱㄱ → ㄱㄱ`으로 입력됩니다.
Shift로 직접 입력하는 쌍자음과 겹받침 조합에는 영향을 주지 않습니다.
...
연결은 업데이트 확인·다운로드에만 사용하며, 키 입력을 기록하거나 전송하지 않습니다.
접근성 권한을 사용하지 않습니다.
