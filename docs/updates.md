...
초기 기록 시점에는 메뉴바 클릭, 새 버전 발견 안내, 관리자 인증을 거친 PKG
업데이트, 취소 후 재시도 및 업데이트 후 실제 입력 앱의 재활성화를 검증하지 않았다.

### 후속 검증

- 서명·공증된 0.6.0(빌드 7) 설치 후 0.6.1-rc.1(빌드 8) 업데이트를 사용자가
  확인했고, 설치된 버전과 시스템 설치 경로의 새 프로세스를 확인했다.
- 운영 공개키와 Keychain 서명 키 일치, PKG 공증·staple·Gatekeeper 검증 및
  공개 GitHub PKG 다운로드 바이트 일치를 확인했다.
- 로그아웃 안내를 포함한 소스에서 테스트 34개와 앱 빌드를 통과했다.
  로그아웃 화면 확인은 사용자 보고이며, 에이전트가 실제 로그아웃을 실행한 것은 아니다.
- 모든 취소·재시도·오류 경로와 설치 후 앱별 입력 호환성을 실기 검증한 것은
  아니다. 실제 검증 범위는 [compatibility.md](compatibility.md)를 따른다.

## 업데이트 후 로그아웃 안내

이 기능이 포함된 버전에서 Sparkle 설치를 진행하면 대상 빌드 번호를 기록한다.
재실행한 앱의 빌드가 대상과 일치할 때 ‘나중에’와 ‘지금 로그아웃’ 안내를 한 번
표시한다. ‘나중에’가 기본 선택이며, 새 버전이 바로 적용되었다면 로그아웃하지
않고 사용할 수 있다. ‘지금 로그아웃’은 작업 저장을 안내한 뒤 macOS의 일반
로그아웃 확인 절차를 요청한다. 강제 종료하거나 즉시 세션을 종료하지 않는다.
요청이 실패하면 Apple 메뉴를 통한 수동 로그아웃을 안내한다.

설치 취소·실패 또는 대상 빌드와 다른 앱 실행에서는 설치 완료 안내를 표시하지
않는다. 이전 버전에는 설치 대상 기록 기능이 없으므로, 이 기능을 처음 설치하는
업데이트에서는 안내가 나오지 않을 수 있다. PKG를 직접 설치한 경우에도 별도
안내를 표시하지 않는다. 안내를 미뤘다면 필요할 때 Apple 메뉴에서 로그아웃한다.
